...
The following **Credential IDs** are expected by the `Jenkinsfile`:
...
1.  **Checkout:** Pulls the latest code from the repository using `pk-github` credentials.
2.  **Build:** Compiles the application (e.g., Maven, Gradle, or NPM).
3.  **Test:** Executes unit tests to ensure code quality and stability.
4.  **Docker Build & Push:** Packages the app into a Docker image and pushes it to Docker Hub using `docker-hub-creds`.
5.  **Deployment:** Connects to the AWS environment via `ec2-ssh-key` to deploy the latest containerized version.
...
2.  **Set up Credentials** in your Jenkins instance as listed in the table above.
3.  **Create a New Job:** In Jenkins, select "Pipeline".
4.  **Configure SCM:** * Set the Pipeline definition to **Pipeline script from SCM**.
    * Enter your repository URL and select the `pk-github` credentials.
5.  **Run Build:** Click "Build Now" to trigger the first run.
